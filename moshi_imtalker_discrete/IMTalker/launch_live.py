...
class FMP4StreamSession:
    ...
    def __init__(
        ...
        self._last_rendered_frame = idle_frame_uint8
        self.fps = fps
        self.debug = debug
    ...
    # ------------------------------------------------------------------ #
    def push_chunk(self, frames_np: np.ndarray, pcm48_int16: np.ndarray) -> None:
        """Called from the render worker (asyncio loop thread).

        Frames: [N,H,W,3] uint8. PCM: 1D int16 at 48 kHz mono.
        """
        with self._play_lock:
            self._play_queue.append([frames_np, pcm48_int16, 0])
    ...
    async def _produce(self) -> None:
        ...
        pace_frame_no = 0
        current = None  # [frames_np, pcm48_int16, pos]
        try:
            while not self._stop.is_set():
                ...
                if current is not None:
                    frames_np, pcm48, pos = current
                    if pos >= frames_np.shape[0]:
                        current = None
                        continue
                    vf = frames_np[pos]
                    self._last_rendered_frame = vf
                    a_start = pos * spf_a48
                    ...
                    with self._play_lock:
                        reply_active = self._reply_active
                    if reply_active:
                        vf = self._last_rendered_frame
                        ap = silence
                        if self._last_mode != "reply_hold" and self.debug:
                            now = time.monotonic()
                            print(
                                f"[DEBUG/fmp4] mode -> reply_hold | "
                                f"reply_for={now - self._last_mode_change_t:.3f}s "
                                f"play_q={len(self._play_queue)} av_q={self.av_q.qsize()}"
                            )
                            self._last_mode_change_t = now
                        self._last_mode = "reply_hold"
                    else:
...
def _add_video_stream(container, fps: int):
