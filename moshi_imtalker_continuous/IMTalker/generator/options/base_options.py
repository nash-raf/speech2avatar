...
class BaseOptions():
	...
	def initialize(self, parser):
		...
		parser.add_argument('--audio_subdir', type=str, default='audio_continuous')
		parser.add_argument('--audio_token_rate', type=float, default=12.5)
		parser.add_argument('--audio_feat_dim', type=int, default=512)
		...
		parser.add_argument('--aux_joint_dropout_prob', default=0.0, type=float,
			help='Training-only: with this probability, jointly zero gaze+pose+cam '
				 'and their prev counterparts per sample. Use 0.5 when fine-tuning '
				 'a model that will be deployed audio-only with zero-aux inputs at '
				 'inference. Does not affect validation or sample().')
		parser.add_argument('--freeze_non_audio', action='store_true')
