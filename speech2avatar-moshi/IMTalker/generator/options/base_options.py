...
class BaseOptions:
    ...
    def initialize(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="audio",
            help="Subdirectory under dataset_path for audio features (audio=wav2vec, audio_mimi=Mimi 512).",
        )
        parser.add_argument(
            "--audio_feat_dim",
            type=int,
            default=768,
            help="Width of audio features loaded from dataset.",
        )
        parser.add_argument("--only_last_features", action="store_true", default=True)
        parser.add_argument("--no_only_last_features", dest="only_last_features", action="store_false")
        parser.add_argument("--average_emotion", action="store_true", help="averaging emotion or not.")
        parser.add_argument(
            "--audio_adapter_mode",
            type=str,
            default="none",
            choices=["none", "bridge_to_768", "temporal_conv_to_32"],
            help="none=identity; bridge_to_768=MLP(audio_feat_dim -> 768) before "
            "audio_projection(768 -> 32); temporal_conv_to_32=temporal conv adapter "
            "producing 32-d conditioning directly.",
        )
        parser.add_argument("--adapter_hidden_dim", type=int, default=1024, help="Hidden width for bridge_to_768.")
        parser.add_argument(
            "--adapter_conv_channels",
            type=int,
            default=256,
            help="Channel width for temporal_conv_to_32.",
        )
        parser.add_argument(
            "--adapter_conv_kernel",
            type=int,
            default=5,
            help="Kernel size for temporal_conv_to_32.",
        )
        parser.add_argument(
            "--bridge_ckpt",
            type=str,
            default=None,
            help="Path to a pretrained audio_adapter state_dict.",
        )
        ...
        parser.add_argument(
            "--freeze_generator",
            action="store_true",
            help="Freeze everything in FMGenerator except the adapter.",
        )
        parser.add_argument(
            "--freeze_audio_projection",
            action="store_true",
            help="Freeze audio_projection only.",
        )
        parser.add_argument(
            "--unfreeze_early_adaln",
            action="store_true",
            help="Unfreeze adaLN_modulation in FMT blocks 0 and 1.",
        )
        ...
        parser.add_argument(
            ...
            type=float,
            default=1.0,
            help="Exponent p in the peak-weighted loss.",
        )
        parser.add_argument(
            "--ema_start_step",
            type=int,
            default=0,
            help="Step at which EMA shadow tracking begins.",
        )
